...
# Creating the SQLlite database to maintain tables and users
def init_sqlite_db():
    """Initialize SQLite database, create tables, and add columns if they don't exist."""
...
def register():
    try:
        ...
        contact_link = post_data.get('contact_link', '')

        with sqlite3.connect('database.db') as con:
            cur = con.cursor()
            cur.execute("INSERT INTO users (username, password, email, contact_link) VALUES (?, ?, ?, ?)", (username, password, email, contact_link))
            con.commit()
            msg = "Congratulations!! You have successfully registered on Github Profiler."
...
# Saves the profile into the mysqllite database
...
# Gitub search function that gets the requests from the api using POST.
...
def github_search():
    ...
    headers = {
<<<<<<< HEAD
        'Authorization': 'token ghp_b************************' # Insert your token here to authenicate every request.
=======
        'Authorization': 'token ghp_b****************************' # Insert your token here to authenicate every request.
>>>>>>> 4d0c720e92b8672c44086b1d2ecb50af7efe4aa8
    }
...
# Function that checks if a user is verified.
